...
export async function updateUserAfterOrder(
  userId: string,
  amount: number,
  order: { _id: string },
  isSuccess = true
  ...
  try {
    ...
    const existingUser = await client.fetch(
      `*[_id == $userId][0]`, // Fetch the user document
      { userId }
    );
    ...
    if (existingUser) {
      ...
      const newUser = await client.create({
        _id: userId,
        _type: "user",
        orders: [{ _type: "reference", _ref: order._id, _key: uuidv4() }],
        orderCount: 1,
        totalSpent: isSuccess ? amount : 0,
      });
...
export async function updateTransaction(customerId: string, transaction: {_id: string}) {
  try {
     await client
